...
class CaptionModel(nn.Module):
    ...
    def forward(self, *args, **kwargs):
        ...
        if 'mode' in kwargs:
            del kwargs['mode']
        # will call function like: self._forward, or self._sample in validation 
    ...
    def beam_search(self, init_state, init_logprobs, *args, **kwargs):
        ...
        def beam_step(logprobsf, unaug_logprobsf, beam_size, t, beam_seq, beam_seq_logprobs, beam_logprobs_sum, state):
            ...
            for vix in range(beam_size):
                ...
                vq_record.append(v['q'])
            state = new_state
            return beam_seq,beam_seq_logprobs,beam_logprobs_sum,state,candidates,vq_record
        ...
        for t in range(self.seq_length + group_size - 1):
            for divm in range(group_size): 
                if t >= divm and t <= self.seq_length + divm - 1:
                    # add diversity
                    logprobsf = logprobs_table[divm].data.float()  # float()
                    ...
                    state_table[divm],\
                    candidates_divm,vq_record = beam_step(logprobsf,
                                                unaug_logprobsf,
                                                bdash,
                                                t-divm,
                                                beam_seq_table[divm],
                    ...
                    logprobs_table[divm], state_table[divm] = self.get_logprobs_state(it.cuda(), *(args[divm][:-2] + [state_table[divm]] + args[divm][-2:])) # the last 2 args are sg embeddings
